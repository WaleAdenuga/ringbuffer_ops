...
impl<T> RingBuffer<T> {
    ...
    pub fn try_write(&self, value: T) -> Result<(), RingBufferError> {
        ...
        if current_seqno == current_head {
            // I own this slot from the consumer, I can write
            let slot_value = self.buffer[slot_index].value.as_ptr() as *mut MaybeUninit<T>;
            unsafe {
    ...
    pub fn try_read(&self) -> Result<T, RingBufferError> {
        loop {
            ...
            let current_seqno = self.buffer[slot_index].seqno.load(Ordering::Acquire);
            if current_seqno == current_tail + 1 { // ready for consumers
...
use super::*; // parent module
                ...
                loop {
                    match cons_buffer.try_read() {
                        Ok(value) => {
                            ...
                            let previous = cons_consumed.fetch_add(1, Ordering::Relaxed);

                            if previous % 10_000 == 0 {
                                println!(
                                    "{:?} consumed {}",
                                    thread::current().id(),
                                    previous + 1
    ...
    fn spawn_producer(buffer: Arc<RingBuffer<u64>>, limit: usize, sleep: bool) -> JoinHandle<()> {
        thread::spawn(move || {
            for i in 0..limit {
                loop { // actual retry mechanism
                    match buffer.try_write(i as u64) {
                        Ok(_) => {
                            if sleep {
    ...
    fn two_consumers() {
        ...
        let producer = spawn_producer(prod_buffer, no_of_writes, false);
        ...
        let consumers = spawn_consumers(cons_buffer, consumed, 2, no_of_writes);
    ...
    fn eight_consumers() {
        ...
        let consumers = spawn_consumers(cons_buffer, consumed, 8, no_of_writes);
    ...
    #[test]
    fn consumer_contention() {
        // create atomically reference counted object --> essentially a shared pointer?
        let buffer = Arc::new(RingBuffer::<u64>::new());

        let no_of_writes = 60_000;
        ...
        // Create a producer that's intentionally slowed so consumers can contend
        let producer = spawn_producer(prod_buffer, no_of_writes, true);
